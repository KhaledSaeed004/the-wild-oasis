...
function Modal({ children }) {
    ...
    return (
        <ModalContext.Provider
            value={{ currOpenWindow, openWindow, closeWindow }}
        >
            {children}
        </ModalContext.Provider>
    );
...
function OpenBtn({ children, opens: windowName }) {
    ...
    if (Array.isArray(children)) {
        console.warn(
            "Only first child element will be considered the modal open button, others will be ignored."
        );
        children = children[0];
    }
...
function Window({ children, name }) {
    ...
    if (Array.isArray(children)) {
        console.warn(
            "Only first child element will be considered the modal window content, others will be ignored."
        );
        children = children[0];
    }
